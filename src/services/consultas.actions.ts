...
export async function updateConsultaStatusAction(id: string, status: ConsultaStatus): Promise<ActionResult> {
  ...
  const { data: updated, error } = await db.from("consultas").update({
    status,
    confirmada_em: status === "confirmada" ? new Date().toISOString() : undefined,
  }).eq("id", id).select("auth_id,data").maybeSingle();
  if (error) return { success: false, message: `Erro ao atualizar consulta: ${error.message}` };

  if (status === "realizada" && updated?.auth_id) {
    const agora = new Date();
    const { data: paciente } = await db.from("pacientes").select("id,fluxo_etapa,reconsulta_intervalo_dias").eq("auth_id", updated.auth_id).maybeSingle();
    if (paciente) {
      const intervalo = Number(paciente.reconsulta_intervalo_dias) || 45;
      const base = updated.data ? new Date(updated.data) : agora;
      ...
      if (paciente.fluxo_etapa !== "06_plano_elaboracao") {
        await db.from("fluxo_movimentacoes").insert({
          ...
          admin_id: null,
          observacao: `Consulta realizada. Reconsulta sugerida em ${intervalo} dias.`,
        });
      }
    }
  }
