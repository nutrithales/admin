...
export async function listCheckinAutomacoes(): Promise<CheckinAutomacao[]> {
  ...
  const { data, error } = await supabase
    ...
    .select("id,formulario_id,nome,publico,paciente_ids,recorrencia_dias,primeira_execucao_em,proximo_disparo_em,ultimo_disparo_em,ativo,formulario:formularios(id,nome,tipo)")
...
export async function getCheckinDashboardResumo() {
  ...
  const [respondidos, pendentes, revisar, proximos] = await Promise.all([
    supabase.from("formulario_envios").select("id", { count: "exact", head: true }).eq("status", "respondido"),
