...
export async function GET() {
  try {
    ...
    const response = await fetch(`${baseUrl}/api/admin`, { headers: { Authorization: `Bearer ${password}` }, cache: "no-store" });
    const body = await response.json().catch(() => ({}));
    if (!response.ok) return NextResponse.json({ success: false, message: body.message ?? body.error ?? "A agenda recusou o acesso." }, { status: response.status });
    ...
    const { data: patients } = authIds.length ? await admin.from("pacientes").select("id, auth_id, nome").in("auth_id", authIds) : { data: [] };
    ...
    return NextResponse.json({
      ...
      events: (body.events ?? []).map((event: { id: string }) => {
        ...
        return { ...event, consultationId: consultation?.id ?? null, status: consultation?.status ?? "agendada", patientId: patient?.id ?? null };
      }),
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const { baseUrl, password } = agendaConfig();
    const response = await fetch(`${baseUrl}/api/admin?id=${encodeURIComponent(id)}`, { method: "DELETE", headers: { Authorization: `Bearer ${password}` }, cache: "no-store" });
    const body = await response.json().catch(() => ({}));
    if (!response.ok) return NextResponse.json({ success: false, message: body.message ?? body.error ?? "Não foi possível cancelar o agendamento." }, { status: response.status });
    const admin = createAdminClient();
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (!id || !CONSULTA_STATUS.includes(body.status)) return NextResponse.json({ success: false, message: "Atualização inválida." }, { status: 400 });

    const admin = createAdminClient();
    const db = admin as any;
    const confirmadaEm = body.status === "confirmada" ? new Date().toISOString() : undefined;
    const { data: updated, error } = await db.from("consultas").update({ status: body.status, confirmada_em: confirmadaEm }).eq("google_event_id", id).select("auth_id").maybeSingle();
    ...
    if (!updated) {
      ...
      if (!email && !phone) return NextResponse.json({ success: false, message: "Este evento ainda não está vinculado a um paciente (sem e-mail/WhatsApp no agendamento). Cadastre ou vincule o paciente manualmente." }, { status: 404 });
      ...
      if (!patient) return NextResponse.json({ success: false, message: "Nenhum paciente cadastrado corresponde a este agendamento. Cadastre o paciente manualmente e tente novamente." }, { status: 404 });
    ...
    if (body.status === "realizada" && authId) {
      ...
      const { data: paciente } = await db.from("pacientes").select("id,fluxo_etapa").eq("auth_id", authId).maybeSingle();
      if (paciente) {
        await db.from("pacientes").update({
          fluxo_etapa: "06_plano_elaboracao",
          fluxo_proxima_acao_em: null,
          plano_entregue_em: null,
          proxima_reconsulta_prevista: null,
        ...
        if (paciente.fluxo_etapa !== "06_plano_elaboracao") {
          await db.from("fluxo_movimentacoes").insert({
            ...
            admin_id: null,
            observacao: "Consulta realizada pela agenda. Reconsulta será calculada após a entrega do plano.",
          });
        }
      }
    }
