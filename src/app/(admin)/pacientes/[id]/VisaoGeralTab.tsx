...
function InfoItem({
  ...
  return (
    <div className="flex min-w-0 items-start gap-3 rounded-lg border border-border bg-white p-3">
      <div className="rounded-full bg-brand-light p-2 text-brand-dark">
  ...
  return (
    <div className="flex flex-col gap-6">
      <section>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-3">
          ...
          <InfoItem
            ...
            value={
              paciente.sexo_biologico === "feminino"
                ? "Feminino"
                : paciente.sexo_biologico === "masculino"
                  ? "Masculino"
                  : "—"
            }
          />
          ...
          <InfoItem
            ...
            value={paciente.treino_frequencia_semanal != null ? `${paciente.treino_frequencia_semanal}x/semana` : "—"}
          />
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        <Card>
          <CardContent className="pt-6">
            <h3 className="font-bold text-ink">Restrições alimentares</h3>
            {restricoes.length > 0 ? (
              ...
              <p className="mt-2 text-sm text-muted">Nenhuma restrição cadastrada.</p>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="pt-6">
            <h3 className="font-bold text-ink">Preferências alimentares</h3>
            <p className="mt-2 whitespace-pre-wrap text-sm text-muted">
              {paciente.preferencias_alimentares || "Nenhuma preferência cadastrada."}
            </p>
          </CardContent>
        </Card>
      </div>
      ...
      <section>
        ...
        {consultas.length === 0 ? (
          <Card>
            <CardContent className="py-8 text-center text-sm text-muted">Nenhuma consulta registrada para este paciente.</CardContent>
          </Card>
        ) : (
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-ink">
                          {consulta.data
                            ? new Date(consulta.data).toLocaleString("pt-BR", { dateStyle: "medium", timeStyle: "short" })
                            : "Consulta sem data"}
                        </p>
                        <p className="text-sm text-muted">
                          {consulta.tipo === "presencial" ? "Presencial" : "On-line"}
                          {consulta.prontuario ? " · prontuário registrado" : ""}
                        </p>
                      </div>
                    </div>
                    <Badge tone={CONSULTA_STATUS_TONE[status] ?? "muted"}>
                      {CONSULTA_STATUS_LABEL[status] ?? consulta.status ?? "Sem status"}
                    </Badge>
                  </CardContent>
                </Card>
              );
