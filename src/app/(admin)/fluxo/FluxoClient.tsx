...
export function FluxoClient({ porEstagio }: { porEstagio: Record<FluxoEstagio, PacienteNoFluxo[]> }) {
  const router = useRouter();
  const { toast } = useToast();
  const [historico, setHistorico] = useState<PacienteNoFluxo | null>(null);

  async function mover(paciente: PacienteNoFluxo, novoEstagio: FluxoEstagio) {
    const result = await moverPacienteFluxoAction(paciente.id, novoEstagio);
    toast({ kind: result.success ? "success" : "error", title: result.message });
    router.refresh();
  }

  const total = Object.values(porEstagio).reduce((acc, list) => acc + list.length, 0);

  return (
    <div>
      <PageHeader
        title="Fluxo de pacientes"
        description={`${total} paciente(s) nas etapas do acompanhamento. Mova o paciente na etapa e o histórico é registrado automaticamente.`}
      />

      <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-thin">
        {FLUXO_ESTAGIOS.map((estagio) => {
          const pacientes = porEstagio[estagio] ?? [];
          return (
            <div key={estagio} className="flex w-72 shrink-0 flex-col rounded-lg border border-border bg-bg-alt-2">
              <div className="flex items-center justify-between border-b border-border px-3.5 py-2.5">
                <h2 className="text-sm font-bold text-ink">{FLUXO_ESTAGIO_LABEL[estagio]}</h2>
                  ...
                  pacientes.map((paciente) => (
                    <div key={paciente.id} className="rounded-md border border-border bg-surface p-2.5 shadow-card">
                      ...
                      <Select
                        ...
                        value={estagio}
                        onChange={(e) => void mover(paciente, e.target.value as FluxoEstagio)}
                      >
