...
type MealGroup = { title: string; options: MealOption[] };
...
export default function DiaryDashboardClient({ patientId, patientName, title, targets, mealData, initialRecords }: {
  ...
  const totals = useMemo(() => totalsFor(dayData, mealData), [dayData, mealData]);
  const mealIds = Object.keys(mealData);
  ...
  const weekTotals = recordedWeek.map((r) => totalsFor(r.data, mealData));
  ...
  const pastWeeks = Array.from({ length: 4 }, (_, index) => {
    ...
    const vals = days.map((d) => records[dateKey(d)]).filter(Boolean).map((d) => totalsFor(d, mealData));
    ...
    <main className="min-h-screen bg-[#F7F5EE] text-[#1F2B24]">
      <div className="mx-auto w-full max-w-[520px] px-4 pb-24 pt-[calc(16px+env(safe-area-inset-top))]">
        ...
        <section className="mb-4 overflow-hidden rounded-[22px] bg-[#15442B] p-5 text-white shadow-[0_14px_35px_rgba(21,68,43,.18)]">
          ...
          <p className="mt-4 border-t border-white/10 pt-3 text-[11px] leading-5 text-white/70">{doneMeals === 0 ? "Marque suas refeições conforme o dia for passando." : doneMeals < mealIds.length ? `${doneMeals} de ${mealIds.length} refeições registradas até agora.` : "Dia completo registrado. Constância é o que constrói resultado, não perfeição."}</p>
        </section>
        ...
        <section className="mb-4 rounded-[18px] bg-white p-4 shadow-sm">
          ...
          <div className="grid grid-cols-7 gap-1">{weekRows.map((row) => {
            const t = totalsFor(row.data, mealData); const future = row.date > new Date(); const has = Boolean(row.data && Object.keys(row.data).length); const ok = has && t.kcal <= Number(targets.kcal) * 1.05;
        ...
        <div className="space-y-3">
          {Object.entries(mealData).map(([groupId, group]) => {
            ...
            return <section key={groupId} className="overflow-hidden rounded-[18px] bg-white shadow-sm">
              <button onClick={() => setOpenGroups((o) => ({ ...o, [groupId]: !o[groupId] }))} className="flex w-full items-center gap-3 p-4 text-left"><span className={`grid size-6 shrink-0 place-items-center rounded-full border-2 text-xs font-black ${selected ? "border-[#19DD7F] bg-[#19DD7F] text-[#15442B]" : "border-[#E7E2D4]"}`}>{selected ? "✓" : ""}</span><span className="min-w-0 flex-1"><b className="block text-sm text-[#1F2B24]">{group.title}</b><span className="block truncate text-[11px] text-[#87938B]">{custom ? `${custom.text} · ${round(custom.kcal)} kcal` : selectedOpt?.label || "Toque para marcar"}</span></span><ChevronDown className={`size-4 text-[#87938B] transition ${isOpen ? "rotate-180" : ""}`} /></button>
