  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 pb-28 sm:space-y-6 sm:pb-8">
      <section className="rounded-[24px] bg-ink-deep p-4 text-white shadow-dark sm:rounded-[28px] sm:p-7">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <p className="text-[11px] font-bold uppercase tracking-[0.16em] text-brand sm:text-xs">Seu plano alimentar</p>
            <h1 className="mt-1 line-clamp-2 text-xl font-black leading-tight sm:text-3xl">{plano.titulo}</h1>
            <p className="mt-1.5 text-xs text-white/65 sm:text-sm">{plano.protocoloNome ?? "Plano individualizado"}</p>
          </div>
        </div>
          ...
          <section id={slugRefeicao(refeicao.id)} key={refeicao.id} className="scroll-mt-4 overflow-hidden rounded-[22px] border border-border bg-surface shadow-card sm:rounded-[24px]">
            <div className="border-b border-border px-4 py-3.5 sm:px-5 sm:py-4">
              <div className="flex items-center justify-between gap-3">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-[0.14em] text-brand-dark sm:text-xs">Refeição</p>
                  <h2 className="mt-0.5 text-lg font-black text-ink sm:text-xl">{refeicao.nome}</h2>
                </div>
                {refeicao.opcoes.length > 1 && <span className="rounded-full bg-brand-light px-2.5 py-1 text-[11px] font-bold text-brand-dark sm:px-3 sm:text-xs">{refeicao.opcoes.length} opções</span>}
              </div>

              {refeicao.opcoes.length > 1 && (
                <div className="-mx-1 mt-3 flex snap-x gap-2 overflow-x-auto px-1 pb-1 scrollbar-thin sm:mt-4">
                  {refeicao.opcoes.map((item) => (
                    <button
                      key={item.numero}
                      onClick={() => setOpcaoPorRefeicao((prev) => ({ ...prev, [refeicao.id]: item.numero }))}
                      className={`min-h-11 shrink-0 snap-start rounded-full px-4 py-2 text-left text-sm font-bold transition active:scale-[0.98] ${item.numero === opcaoAtiva ? "bg-ink-deep text-white" : "bg-bg-alt text-muted hover:text-ink"}`}
                    >
                      <span className="block">Opção {item.numero}</span>
                      {item.nome && <span className="block max-w-[220px] truncate text-[11px] font-semibold opacity-75">{item.nome}</span>}
                    </button>
                  ))}
                </div>
              )}
            ...
            <div className="space-y-2.5 p-3.5 sm:space-y-3 sm:p-5">
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        ...
                        {formatarQuantidade(item.quantidadeG, item.quantidadeTexto) && (
                          <p className="mt-1 text-sm font-bold text-brand-dark">{formatarQuantidade(item.quantidadeG, item.quantidadeTexto)}</p>
                        )}
                        {item.ingredientes && item.ingredientes.length > 0 && (
                          <div className="mt-2 space-y-1">
                            {item.ingredientes.map((ing, index) => (
                              <p key={`${ing.nome}-${index}`} className="text-sm leading-5 text-muted">{ing.nome}: {Math.round(ing.quantidadeG)} g</p>
                            ))}
                          </div>
                        )}
                      </div>
                          ...
                          <div className="mt-2 space-y-2">
                            {subs.map((sub, index) => (
                              <div key={`${sub.nome}-${index}`} className="flex items-center justify-between gap-3 rounded-xl bg-white px-3 py-2.5 text-sm">
                                <span className="font-semibold leading-5 text-ink">{sub.nome}</span>
                                <span className="shrink-0 font-black text-brand-dark">{Math.round(sub.quantidadeG)} g</span>
                              </div>
                            ))}
                          </div>
                        )}
              ...
              <div className="rounded-2xl border border-brand/30 bg-brand-light p-3.5 sm:p-4">
              ...
              {refeicao.observacoes && (
                <div className="rounded-2xl border-l-4 border-brand bg-bg-alt p-3.5 sm:p-4">
      ...
      {plano.substituicoes.length > 0 && (
        <section className="rounded-[22px] border border-border bg-surface shadow-card sm:rounded-[24px]">
          <button
          ...
          >
            <span className="flex min-w-0 items-start gap-3">
              ...
              <span className="min-w-0">
                <span className="block text-[11px] font-bold uppercase tracking-[0.14em] text-brand-dark sm:text-xs">Lista ampliada</span>
                <span className="mt-0.5 block text-lg font-black text-ink sm:text-xl">Substituições do plano</span>
                <span className="mt-1 block text-xs leading-4 text-muted sm:text-sm">Todas as alternativas extras calculadas para suas porções.</span>
              </span>
            </span>
                ...
                {plano.refeicoes.map((refeicao) => {
                  ...
                  return (
                    <div key={`lista-${refeicao.id}`}>
                      <h3 className="font-black text-ink">{refeicao.nome}</h3>
                      <div className="mt-2 space-y-3">
                        {blocos.map(({ opcao, item, subs }) => (
                          <div key={`lista-${item.id}`} className="rounded-2xl bg-bg-alt-2 p-3.5 sm:p-4">
                            <p className="text-[10px] font-bold uppercase tracking-[0.1em] text-muted sm:text-xs">Opção {opcao.numero}{opcao.nome ? `: ${opcao.nome}` : ""}</p>
                            <p className="mt-1 text-sm font-bold text-ink sm:text-base">No lugar de {item.nome}</p>
                            <div className="mt-2 space-y-2 sm:grid sm:grid-cols-2 sm:gap-2 sm:space-y-0">
                              {subs.map((sub, index) => (
                                <div key={`final-${item.id}-${index}`} className="flex items-center justify-between gap-3 rounded-xl bg-white px-3 py-2.5 text-sm">
                                  <span className="font-medium leading-5 text-ink">{sub.nome}</span>
                                  <span className="shrink-0 font-black text-brand-dark">{Math.round(sub.quantidadeG)} g</span>
                                </div>
                              ))}
                            </div>
      ...
      {plano.observacoes && (
        <section className="rounded-[22px] border border-border bg-surface p-4 shadow-card sm:rounded-[24px] sm:p-5">
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-brand-dark sm:text-xs">Orientações</p>
          <p className="mt-2 whitespace-pre-line text-sm leading-5 text-ink sm:mt-3 sm:leading-6">{plano.observacoes}</p>
        </section>
      )}

      <nav className="fixed inset-x-0 bottom-0 z-40 border-t border-border bg-white/95 px-2 pb-[max(env(safe-area-inset-bottom),8px)] pt-2 shadow-[0_-8px_24px_rgba(0,0,0,0.08)] backdrop-blur sm:hidden">
        <div className="mx-auto grid max-w-md grid-cols-5">
          <MobileNavItem href="/paciente" icon={Home} label="Início" />
          <MobileNavItem href="/paciente/plano-alimentar" icon={Utensils} label="Dieta" active />
          <MobileNavItem href="/paciente" icon={Dumbbell} label="Treinos" />
          <MobileNavItem href="/paciente" icon={BookOpen} label="Materiais" />
          <MobileNavItem href="/paciente" icon={ClipboardCheck} label="Check-in" />
        </div>
      </nav>
    </div>
  );
...
function Resumo({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-2xl bg-white/8 px-3 py-2.5 sm:py-3">
      <p className="text-[10px] font-bold uppercase tracking-[0.1em] text-white/50 sm:text-[11px] sm:tracking-[0.12em]">{label}</p>
      <p className="mt-0.5 text-sm font-black text-white sm:mt-1 sm:text-base">{value}</p>
    </div>
  );
}

function MobileNavItem({ href, icon: Icon, label, active = false }: { href: string; icon: typeof Home; label: string; active?: boolean }) {
  return (
    <a href={href} className={`flex min-h-12 flex-col items-center justify-center gap-0.5 rounded-xl text-[10px] font-bold ${active ? "text-brand-dark" : "text-muted"}`}>
