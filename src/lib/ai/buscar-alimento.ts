...
export async function buscarAlimentoComIA(nomeAlimento: string): Promise<AlimentoEstimado> {
  ...
  if (!apiKey) throw new Error("ANTHROPIC_API_KEY não configurada — a busca por IA está desabilitada.");
  ...
  const message = await client.messages.create({
    ...
    messages: [{ role: "user", content: `Estime a composição nutricional de: ${nomeAlimento}` }],
    tools: [
      {
        name: TOOL_NAME,
  ...
  const parsed = alimentoEstimadoSchema.safeParse(toolUse.input);
  if (!parsed.success) {
