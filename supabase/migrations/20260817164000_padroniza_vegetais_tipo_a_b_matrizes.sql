...
as $$
  with grp as (
    select id from public.grupos_substituicao
    where codigo='VEG_B' and biblioteca='clinica_v1'
    limit 1
  ), ref as (
    select a.id
    from public.alimentos a
    join public.grupo_substituicao_itens i on i.alimento_id=a.id
    cross join grp
    where i.grupo_id=grp.id and a.nome='Cenoura cozida'
    limit 1
  )
  select 'Vegetais Tipo B — 1 porção: ' ||
    string_agg(a.nome || ' — ' || trim(to_char(public.quantidade_equivalente_grupo(grp.id, ref.id, 100, a.id),'FM999990')) || ' g', ' | ' order by i.ordem) ||
    '. Escolha 1 opção da lista.'
  from grp cross join ref
  join public.grupo_substituicao_itens i on i.grupo_id=grp.id
  join public.alimentos a on a.id=i.alimento_id and a.ativo=true;
$$;
...
declare
  ...
  v_opcao integer;
  v_nome text;
  v_tem_tipo_a boolean := false;
  v_tem_tipo_b boolean := false;
begin
  ...
  select exists(
    select 1 from public.protocolo_refeicoes pr
    join public.planos_estruturados pe on pe.protocolo_id=pr.protocolo_id
    join public.protocolo_refeicoes_preferidas pref on pref.protocolo_refeicao_id=pr.id
    join public.refeicoes_modelo rm on rm.id=pref.refeicao_modelo_id
    join public.refeicao_modelo_opcoes o on o.refeicao_modelo_id=rm.id
    join public.refeicao_modelo_opcao_itens i on i.opcao_id=o.id and i.papel_macro='livre'
    where pe.id=new.plano_estruturado_id and pr.nome=new.nome
  ) into v_tem_tipo_a;
  ...
  for v_opcao, v_nome in
    select opcao_numero, coalesce(max(opcao_nome),'Opção '||opcao_numero)
    from public.plano_refeicao_itens where plano_refeicao_id=new.id
    group by opcao_numero order by opcao_numero
  loop
    v_observacoes := v_observacoes || case when v_observacoes='' then '' else E'\n\n' end || 'Opção '||v_opcao||' — '||v_nome||':';

    for v_item in
      select pi.alimento_id,pi.grupo_substituicao_id,pi.quantidade_g,a.nome,g.codigo as grupo_codigo
      from public.plano_refeicao_itens pi
      join public.alimentos a on a.id=pi.alimento_id
      left join public.grupos_substituicao g on g.id=pi.grupo_substituicao_id
      where pi.plano_refeicao_id=new.id and pi.opcao_numero=v_opcao and pi.grupo_substituicao_id is not null
        and coalesce(g.codigo,'') not in ('VEG_A','VEG_B')
      order by pi.ordem
    loop
      ...
      from (
        ...
        from public.grupo_substituicao_itens gsi
        join public.alimentos a2 on a2.id=gsi.alimento_id and a2.ativo=true
      ...
      if v_subs is not null then
        v_observacoes := v_observacoes || E'\n' || 'Substituições para '||v_item.nome||' ('||trim(to_char(v_item.quantidade_g,'FM999990'))||' g): '||v_subs||'.';
      end if;
    end loop;
  end loop;
